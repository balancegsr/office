...
- **智能内容引导**：不只是接收指令——素材不足时 Agent 会主动引导你补充关键信息，素材充足时自动结构化，全程对话完成
- **14 种布局**：标题页、内容页、双栏、卡片、大数字、引用、图片、图表、时间线、对比、金字塔等，根据内容特征自动选择最优布局
- **16 套主题 + 可视化选择**：8 种风格 × 浅色/深色（含液态玻璃/磨砂玻璃/渐变），通过浏览器内可视化选择器预览并点选，所见即所得
- **风格提取**：支持从 .pptx 文件、网页链接、图片截图、PDF 中提取配色和排版特征，自动生成匹配风格
- **数据可视化**：SVG 图表、Chart.js、ECharts 三种方案，Agent 根据数据特征自动选择，翻页时动画精准触发
- **入场动画**：CSS 基础动画 + 可选 GSAP 高级动画（数字滚动、打字机、路径动画等）
- **完整交互**：键盘（← → Space）、鼠标滚轮、触控滑动翻页，右侧圆点导航（可点击跳转），右上角全屏按钮（F 键）
- **视觉 QA**：生成后自动逐页检查溢出、遮挡、色值一致性、布局多样性等，确认无问题才交付
- **单文件交付**：生成的 HTML 自包含所有样式和脚本，浏览器直接打开即可演示，发送给任何人都能打开
